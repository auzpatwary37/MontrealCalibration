...
public class RouteConsistencyChecker implements BeforeMobsimListener,StartupListener{
	...
	public void notifyStartup(StartupEvent event) {
		this.scneario = event.getServices().getScenario();
		this.net = this.scneario.getNetwork();
				...
				if(pl instanceof Leg) {
					...
					if(l.getRoute() instanceof NetworkRoute) {
						...
						for(int i = 0; i<allTurns.size()-1;i++) {
							if(this.prohibitedTurns.get(allTurns.get(i))!=null && this.prohibitedTurns.get(allTurns.get(i)).contains(allTurns.get(i+1))) {
